...
# Appended after every product's own photos: a size chart and two shop-trust
# cards the seller wants on every listing. Missing gracefully (rather than
# raising) so a package can still be prepared before these assets exist.
FOOTER_IMAGES_DIR = Path(__file__).resolve().parents[2] / "assets" / "brand_footer_images"
...
def prepare_listing_package(
    ...
    *,
    ...
    if download_images:
        image_files += _append_footer_images(folder, start_index=len(image_files) + 1)
